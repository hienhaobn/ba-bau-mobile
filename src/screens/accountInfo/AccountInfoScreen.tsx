...
import { fetchProfile, fetchUpdate } from '../../states/user';
...
const AccountInfoScreen = () => {
    ...
    useEffect(() => {
        ...
        if (userInfo?.fullname) {
            setMomName(userInfo?.fullname);
        }
        if (userInfo?.address) {
            setMomAddress(userInfo?.address);
        }
        if (userInfo?.phone) {
            setPhone(userInfo?.phone);
        }
        if (userInfo?.childName) {
            setBabyName(userInfo?.childName);
        }
        if (userInfo?.birthday) {
            setMomDOB(moment(userInfo?.birthday, 'DD-MM-YYYY').toDate())
        }
        if (userInfo?.childBirthday) {
            setDueDate(moment(userInfo?.childBirthday, 'DD-MM-YYYY').toDate())
        }
        if (userInfo?.lastMenstrualPeriod) {
            setLastMenstrualPeriod(moment(userInfo?.lastMenstrualPeriod, 'DD-MM-YYYY').toDate());
        }
    }, [userInfo])
    ...
    const renderInputMomPhone = () => (
        <View style={styles.inputContainer}>
            ...
            <Input value={phone} onChangeText={setPhone} placeholder='Vui lòng nhập số điện thoại' editable={isEdit} />
        </View>
    );
    ...
    return (
        <View style={styles.container}>
            <Header title='Thông tin tài khoản' containerStyle={{ backgroundColor: getThemeColor().Color_Primary2 }} iconRight={renderRightHeader()} onPressRight={() => setIsEdit(true)} />
